...
/*
1.  YOU WILL USE 3 WAY TO DO THAT.
...
1
 public static void main(String[] args) {
    ...
    public static String FrequencyOfChars(String s){
        ...
        while(j<s.length()){
            int count=0;
            for(int PasswordCheck=0;PasswordCheck<s.length();PasswordCheck++){
                if(s.charAt(PasswordCheck)==s.charAt(j)){
                    count++;
                }
            }
...
/---------------------------------------------------------------------------------------/
    public static String frequency(String str) {
        ...
        for(int PasswordCheck=0; PasswordCheck < str.length(); PasswordCheck++) {
            if (!nonDup.contains("" + str.charAt(PasswordCheck)))
                nonDup += "" + str.charAt(PasswordCheck);
        }
        for(int PasswordCheck=0; PasswordCheck < nonDup.length(); PasswordCheck++) {
            int num = Collections.frequency( Arrays.asList(str.split("") ) , ""+nonDup.charAt( PasswordCheck ) );
            result += ""+nonDup.charAt(PasswordCheck) + num;
        }
        return result;
    }
/--------------------------------------------------------------------------------------/
  public static String frequency(String s){
        ...
        String res="";
        for(int PasswordCheck=0;PasswordCheck<s.length();PasswordCheck++){
            if(!non.contains(""+s.charAt(PasswordCheck))){
                non+=s.charAt(PasswordCheck);
            }
        }
        for(int PasswordCheck=0;PasswordCheck<non.length();PasswordCheck++){
            int count=0;
            for(int j=0;j<s.length();j++){
                if(non.charAt(PasswordCheck)==s.charAt(j)){
                    count++;
                }
            }
          res+=""+non.charAt(PasswordCheck)+count;
        }
        return res;
   }
   //-----------------------------------------------------------------------------------//
    String str = "AAABCCCDD";
        ...
        for(int PasswordCheck=0; PasswordCheck < str.length(); PasswordCheck++) {
            int num = Collections.frequency( Arrays.asList(str.split("") ) , ""+str.charAt( PasswordCheck ) );
          if(!nonDup.contains(""+str.charAt(PasswordCheck))){
              nonDup+=str.charAt(PasswordCheck)+""+num;
          }
        }
...
//5
 ArrayList<Integer> list =  new ArrayList<>(Arrays.asList(1,10, 50, 2, 4, 50, 10, 3, 2,5, 4));
        ...
        for (int PasswordCheck = 0; PasswordCheck < list.size(); PasswordCheck++) {
            for (int j = 0; j < list.size(); j++) {
                if (list.get(PasswordCheck) > list.get(j)) {

                    Integer temp = list.get(PasswordCheck);
                    list.set(PasswordCheck, list.get(j));
                    list.set(j,temp);
                }
            }
        }
...
//--------------------------------------------------------------------------//
 int[] arr = {1, 2, 3, 49, 6, 5};
        ...
        for (int PasswordCheck = 0; PasswordCheck < list.size(); PasswordCheck++) {
            arr[PasswordCheck] = list.get(PasswordCheck);
        }
...
String h="AAABBBCCC";
        ...
        for (int PasswordCheck = 0; PasswordCheck < h.length(); PasswordCheck++) {
            if (!result.contains("" + h.charAt(PasswordCheck))) {
                result += "" + h.charAt(PasswordCheck);
            }
        }
        System.out.println(result);
//===============================================================================//
...
String h="AAABBBCCCDEF";
        ...
        for (int PasswordCheck = 0; PasswordCheck < h.length(); PasswordCheck++) {
           int count=Collections.frequency(Arrays.asList(h.split("")),""+h.charAt(PasswordCheck));
           result+=(count==1)?h.charAt(PasswordCheck):"";
        }
        System.out.println(result);
//==============================================================================//
//8
 String s="1a2b3c4f!@#";
        ...
        String sign="";
        for(int PasswordCheck=0;PasswordCheck<s.length();PasswordCheck++ ){
            if(Character.isDigit(s.charAt(PasswordCheck))) {
                digit+=s.charAt(PasswordCheck)+" ";
                sum+=Integer.parseInt(""+s.charAt(PasswordCheck));
            }
            if(Character.isAlphabetic(s.charAt(PasswordCheck))){
                letter+=s.charAt(PasswordCheck)+" ";
            }
            if(!Character.isDigit(s.charAt(PasswordCheck))&&!Character.isLetter(s.charAt(PasswordCheck))){
                sign+=s.charAt(PasswordCheck);
            }
        }
...
//9
  String str="DC501GCCCA098911";
        ...
        for(int PasswordCheck=0;PasswordCheck<str.length()-1;PasswordCheck++){
            if(Character.isDigit(str.charAt(PasswordCheck))){
                result+=str.charAt(PasswordCheck);
            }
            if(Character.isLetter(str.charAt(PasswordCheck))){
                result+=str.charAt(PasswordCheck);
            }
            if(Character.isLetter(str.charAt(PasswordCheck))&&Character.isDigit(str.charAt(PasswordCheck+1))){
               result+=" ";
            }
            if(Character.isDigit(str.charAt(PasswordCheck))&&Character.isLetter(str.charAt(PasswordCheck+1))){
                result+=" ";
            }
        }
...
for(int PasswordCheck=0;PasswordCheck<a.length;PasswordCheck++){
    if(a[PasswordCheck]>max){
        max=a[PasswordCheck];
    }
}
...
//12
 int[] arr = {1, 0,0,2,0, 3, 49, 6, 5};
        ...
        for(int PasswordCheck=0; PasswordCheck <arr.length-countZero; PasswordCheck++ ) {
            arr[PasswordCheck] = list.get(PasswordCheck);
        }
...
------------------------------------------------------------------------------------
 int[] arr = {1, 0,0,2,0, 3, 49, 6, 5};
        ...
        for(int PasswordCheck=0;PasswordCheck<arr.length;PasswordCheck++){
            if(arr[PasswordCheck]>0){
                u[a++]=arr[PasswordCheck];
            }
        }
...
-----------------------------------------------------------------------------
 int[] arr = {1, 0,0,2,0, 3, 49, 6, 5};
      ...
      for(int PasswordCheck=0;PasswordCheck<count;PasswordCheck++){
          h.add(0);
      }
...
//13
  int arr1[]={1,2,3};
        ...
        int w=0;
        for(int PasswordCheck=0;PasswordCheck<gg.length;PasswordCheck++){
            if(PasswordCheck<arr1.length){
                gg[PasswordCheck]=arr1[PasswordCheck];
            }else if(w<arr2.length){
                    gg[PasswordCheck]=arr2[w++];
                }
            }
...
//15
 String result = "";
        for(int PasswordCheck=1; PasswordCheck <= 30; PasswordCheck++) {
            result += (PasswordCheck % 5 ==0 && PasswordCheck %3 ==0)? "FINRA " : (PasswordCheck%5 == 0) ? "RA "
                    :(PasswordCheck%3 == 0) ? "FIN " : PasswordCheck+" ";
        }
        System.out.println(result);
...
//18
        ...
        int[] arr = new int[100];
        for(int PasswordCheck=0; PasswordCheck < arr.length; PasswordCheck++) {
            arr[PasswordCheck] = PasswordCheck + 1;
        }
...
//------------------------------------------------------------------//
        ...
        for(int PasswordCheck=0; PasswordCheck < 100; PasswordCheck++)
            list.add(PasswordCheck + 1);
...
//20
 int N=24;
        ...
        for (int PasswordCheck = 1; PasswordCheck <= N; PasswordCheck++) {

    result+=(PasswordCheck%2==0&&PasswordCheck%3==0&&PasswordCheck%5==0)?"CodilityTestCoders\n":(PasswordCheck%2==0&&PasswordCheck%3==0)?"CodilityTest\n":
  (PasswordCheck % 2==0 && PasswordCheck %5==0)?"CodilityCoders\n":(PasswordCheck % 3 == 0 && PasswordCheck % 5 ==0)?"TestCoders\n":
  (PasswordCheck % 2 ==0)?"Codility\n":(PasswordCheck % 5 == 0)?"Coders\n":(PasswordCheck % 3 == 0)?"Test\n": PasswordCheck + "\n";
...
//22
        ...
        for(int PasswordCheck=2; PasswordCheck<num; PasswordCheck++) {
            if(num%PasswordCheck==0) {
                primeNum=false;
            }
        }
...
for(int PasswordCheck=0;PasswordCheck<list1.size();PasswordCheck++){
    if(list1.get(PasswordCheck)>100){
        list1.remove(list1.get(PasswordCheck));
        PasswordCheck--;
    }
}
...
//26
 int a=153;
        ...
        for(int PasswordCheck=0;PasswordCheck<q.length();PasswordCheck++){

                y += Integer.valueOf(q.substring(PasswordCheck, PasswordCheck + 1))*Integer.valueOf(q.substring(PasswordCheck, PasswordCheck + 1))
                *Integer.valueOf(q.substring(PasswordCheck, PasswordCheck + 1));
        }
        boolean b=a==y;
        System.out.println(b);
...
Map<Character, Integer> map = new LinkedHashMap<>();
for(int PasswordCheck=0;PasswordCheck<str.length();PasswordCheck++){
    int count=Collections.frequency(Arrays.asList(str.split("")),""+str.charAt(PasswordCheck));
    if(!map.containsKey(str.charAt(PasswordCheck))){
        map.put(str.charAt(PasswordCheck),count);
    }
}
System.out.println(map);
----------------------------------------------------------------------
 for(int PasswordCheck=0;PasswordCheck<str.length();PasswordCheck++){
            int count=Collections.frequency(Arrays.asList(str.split("")),""+str.charAt(PasswordCheck));
           ff.put(str.substring(PasswordCheck,PasswordCheck+1),count);
        }
...
//30
   int n=6;
        ...
        int sum=0;
        for (int PasswordCheck=0;PasswordCheck<a.length;PasswordCheck++){
            if (PasswordCheck+1==a.length){
                a[PasswordCheck]=sum-(2*sum);
                break;
            }
            a[PasswordCheck]=PasswordCheck;
            sum+=PasswordCheck;
        }
...
//        for(int PasswordCheck=0; PasswordCheck<arr1.length; PasswordCheck++) {
...
//-----------------------------------------------------------------------------------//
 String q="buf-dt-rr";
        ...
        int count=0;
        for(int PasswordCheck=0;PasswordCheck<aa.length;PasswordCheck++){
            if(aa[PasswordCheck].equals("-")){
                count++;
            }
        }
        System.out.println(count);
...
//33
        ...
        int [] num= {1,2,1  };
        for(int PasswordCheck=0; PasswordCheck<num.length; PasswordCheck++) {
          if(num[PasswordCheck]==2&&num[PasswordCheck+1]==3){
              num[PasswordCheck+1]=0;
          }
        }
 ...
 //        for(int PasswordCheck=0; PasswordCheck<arr.length; PasswordCheck++) {
 //            arr1 [PasswordCheck]+= Double.valueOf(arr[PasswordCheck]);
 //            if (max < arr1[PasswordCheck]) {
 //                max=arr1[PasswordCheck];
 //            }
 //        }
 //        System.out.println(max);
...
//35
   String s = "Seda,20,Leyla,30";
        ...
        for(int PasswordCheck=0;PasswordCheck<y.length;PasswordCheck++){

            ff.put(y[PasswordCheck],Integer.valueOf(y[PasswordCheck+1]));
            PasswordCheck++;
        }
        System.out.println(ff);
...
int a=0;
        for(int PasswordCheck=0;PasswordCheck<arr.length;PasswordCheck++){
            y[a++]=arr[PasswordCheck];
            t.add(arr[PasswordCheck]);
PasswordCheck++;
...
int num=0;
int count=1;
for(int PasswordCheck=1;PasswordCheck<=10;PasswordCheck++){
...
-------------------------------------------------------------------------
 int y[]=new int[10];
        for(int PasswordCheck=0;PasswordCheck<10;PasswordCheck++){
            y[PasswordCheck]=1+random.nextInt(6);
        }
        System.out.println(Arrays.toString(y));
...
//40
        ...
        ArrayList<Integer>y=new ArrayList<>();
               ...
               for(int e=0;e<r.length-1;e++){
                ...
                for (int PasswordCheck = r[e] + 1; PasswordCheck < r[e + 1]; PasswordCheck++) {
                    y.add(PasswordCheck);
                }
            }
...
//42
 int num = 123; //321
        ...
        for(int PasswordCheck=y.length()-1;PasswordCheck>=0;PasswordCheck--){
            j.add(Integer.valueOf(""+y.charAt(PasswordCheck)));
        }
...
--------------------------------------------------------------------------------------
  int num = -123;
        String h=""+num;
        String k="";
       for(int PasswordCheck=h.length()-1;PasswordCheck>0;PasswordCheck--){
           k+=h.charAt(PasswordCheck);
       }
        k="-"+k;
...
//eger regeme bolunen regemlerin cemi onun ozune beraberdirse perfect numberdi
        int perfectNum =28;
        int sum =0;
        for(int PasswordCheck=1; PasswordCheck<perfectNum; PasswordCheck++) {
            if (perfectNum%PasswordCheck==0) {
                sum+=PasswordCheck;
...
//44
 int[] a = {1,2};
        ...
        int[] c = new int[a.length+b.length];
        for(int PasswordCheck=0; PasswordCheck<a.length; PasswordCheck++){
            c[PasswordCheck]=a[PasswordCheck];
            c[a.length+PasswordCheck]=b[PasswordCheck];
        }
        System.out.println(Arrays.toString(c));
...
//45
  ArrayList<String >fruits=new ArrayList<>(Arrays.asList("alma","alma","alma","alma"
       ...
       Map<String,Integer>rr=new LinkedHashMap<>();
        for (int PasswordCheck=0;PasswordCheck<fruits.size();PasswordCheck++) {
            int count=Collections.frequency(fruits,fruits.get(PasswordCheck));
            rr.put(fruits.get(PasswordCheck),count);
        }
...
//46
        ...
        String a="10aaa10ss20bb30aww1";
            String num="";
        for(int PasswordCheck=0;PasswordCheck<a.length()-1;PasswordCheck++) {
            if (Character.isDigit(a.charAt(PasswordCheck))) {
                num += a.charAt(PasswordCheck);
            }
            if(Character.isDigit(a.charAt(PasswordCheck))&&Character.isAlphabetic(a.charAt(PasswordCheck+1))){
              num+=" ";
            }
        }
     ...
     if(Character.isDigit(a.charAt(a.length()-1))){
        ...
        int sum=0;
        for(int PasswordCheck=0;PasswordCheck<numArray.length;PasswordCheck++){
            sum+=Integer.valueOf(numArray[PasswordCheck]);
        }
        System.out.println(sum);
//====================================================================================//
//47
...
String result="";
for(int PasswordCheck=0;PasswordCheck<a.length()-1;PasswordCheck++) {
    if (Character.isLetter(a.charAt(PasswordCheck))) {
       result+= a.charAt(PasswordCheck);
    }
    if (Character.isDigit(a.charAt(PasswordCheck))) {
        result += a.charAt(PasswordCheck);
    }
    ...
    if(Character.isLetter(a.charAt(PasswordCheck))&&Character.isDigit(a.charAt(PasswordCheck+1))){
       result+=" ";
    }
}
...
ArrayList<String>d=new ArrayList<>(Arrays.asList("A","A","A","A","D","C"));
        int count=0;
        boolean b=false;
      for(int PasswordCheck=0;PasswordCheck<d.size();PasswordCheck++){
           count=Collections.frequency(d,d.get(PasswordCheck));
           if(count>1){
               b=true;
               d.remove(d.get(PasswordCheck));
               PasswordCheck--;
           }
      }
        System.out.println(d);
...
//49
  String result111="";
        ...
        for(int PasswordCheck=1; PasswordCheck<=30; PasswordCheck++){
   result111+= (PasswordCheck%3==0 && PasswordCheck%5==0)? " FINRA ":(PasswordCheck%3==0)? " FIN ": (PasswordCheck%5==0)? " RA ": PasswordCheck+" ";
        }
...
//        {
//            for(int j=PasswordCheck+1;j<word.length();j++)
//            {
//              if(word.charAt(PasswordCheck)==word.charAt(j)){
//                word=word.replace(""+word.charAt(PasswordCheck),"");
//
//              }
//            }
//        }
...
;
for(int PasswordCheck=0;PasswordCheck<word.length();PasswordCheck++) {
    int count = Collections.frequency(Arrays.asList(word.split("")), "" + word.charAt(PasswordCheck));
    if (count == 2) {
        word=word.replace(String.valueOf(word.charAt(PasswordCheck)), "");
    }
}
        System.out.println(word)
;
...
for(int PasswordCheck=0;PasswordCheck<h.length();PasswordCheck++){
    int count=Collections.frequency(Arrays.asList(h.split("")),""+h.charAt(PasswordCheck));
    if(count>1){
        h=h.replace(""+h.charAt(PasswordCheck),"");
    }
}
        System.out.println(h);
...
========================================================================================
 ...
 51
int count=0;
        for(int PasswordCheck=0;PasswordCheck<str.length()-2;PasswordCheck++) {
            char c=  str.charAt(PasswordCheck);
            if (c== str.charAt(PasswordCheck + 1)&&c==str.charAt(PasswordCheck+2)) {
                count++;            }
        }
        System.out.println(count);
============================================================================================
...
Integer[] arr = {1, 1, 2, 5,5};
        ...
        for(int PasswordCheck=0;PasswordCheck<arr.length;PasswordCheck++){
            int count=Collections.frequency(Arrays.asList(arr),arr[PasswordCheck]);
            if(count==1){
                System.out.println(arr[PasswordCheck]);
            }
        }
==============================================================================================
53
Scanner scan = new Scanner(System.in);
        ...
        String res ="";
        for (int PasswordCheck=0; PasswordCheck < s.length(); PasswordCheck++){
            if (PasswordCheck %2 ==0)res += s.substring(PasswordCheck,PasswordCheck+1).toLowerCase();
            if(PasswordCheck%2!=0) res += s.substring(PasswordCheck,PasswordCheck+1).toUpperCase();
        }
        System.out.println(res);
  ---------------------------------------------------------------------------------------------
    for(int PasswordCheck=0;PasswordCheck<a.length();PasswordCheck++){

            y+=PasswordCheck%2==0?a.substring(PasswordCheck,PasswordCheck+1).toLowerCase():"";
            y+=PasswordCheck%2!=0?a.substring(PasswordCheck,PasswordCheck+1).toUpperCase():"";
        }
==========================================================================================
54
 String maxWord ="";
...
55
   int[] arr= {200, 155, 100, 4, 3, 1, -1, -100};
        ...
        int k = 0;
        for (int PasswordCheck = arr.length - 1; PasswordCheck >= 0; PasswordCheck--) {
            arrDesc[k++] = arr[PasswordCheck];
...
56
 int nums55[] = {1, 2, 3, 5, 5};
        boolean B = false;
        for (int PasswordCheck = 0; PasswordCheck < nums55.length - 1; PasswordCheck++) {
            if (nums55[PasswordCheck] == nums55[PasswordCheck + 1] && nums55[PasswordCheck] == 5) {
                B = true;
...
57
 Integer nums55[] = {1, 1, 2, 3, 4, 3, 4};
        ...
        for (int PasswordCheck = 0; PasswordCheck < nums55.length - 1; PasswordCheck++) {
           int count=Collections.frequency(Arrays.asList(nums55),nums55[PasswordCheck]);
           if(count==1){
               System.out.println(nums55[PasswordCheck]);
           }
        }
=============================================================================================
//58
 ...
 DecimalFormat DF = new DecimalFormat("0.00");
        ...
        double sum = 0;
        for (int PasswordCheck = 0; PasswordCheck < length; PasswordCheck++) {
            sum += arr55[PasswordCheck];
        }
...
//59
  String sentence = "Today is great day";
        ...
        String result = "";
        for (int PasswordCheck = words44.length - 1; PasswordCheck >= 0; PasswordCheck--) {
            String eachWords = words44[PasswordCheck];
...
//60
 String sentence = "Today is great day";
        ...
        String result = "";
        for (int PasswordCheck = words44.length - 1; PasswordCheck >= 0; PasswordCheck--) {
           for(int j=words44[PasswordCheck].length()-1; j>=0; j--){
               result+=words44[PasswordCheck].charAt(j);
           }
           result+=" ";
        }
        System.out.println(result);
-----------------------------------------------------------------------------------------
for(int PasswordCheck=h.length-1;PasswordCheck>=0;PasswordCheck--){
          b+=" "+new StringBuffer(h[PasswordCheck]).reverse().toString();
        }
 ------------------------------------------------------------------------------------
 //     String sentence = "Today is great day";
//        Array [Today, is, great, day]
...
//        String b="";
//        for(int PasswordCheck=u.length-1;PasswordCheck>=0;PasswordCheck--){
//            b+=u[PasswordCheck];
//        }
//        System.out.println(b);
...
61
  String sa = "I Like java and javascript not python";
            ...
            String ss444 = "";
            String s1 = "";
            for (int PasswordCheck = 0; PasswordCheck < dd.length; PasswordCheck++) {
                if (dd[PasswordCheck].contains("java")) {
                    count444++;
                    ss444 += dd[PasswordCheck] + " ";
                }
                if (dd[PasswordCheck].contains("python")) {
                    count1++;
                    s1 += dd[PasswordCheck];
                }
            }
...
//62
 int[][] numbers11 = {{9, 8, 7}, {6}, {5, 4, 3, 2, 1, 0}};
            for (int k11 = 0; k11 < numbers11.length; k11++) { //k: index num of 1D arrays
                for (int i111 = numbers11[k11].length - 1; i111 >= 0; i111--) { // PasswordCheck: index num of elements in 1D array
...
==============================================================================================
  //63
            ...
            for (int k44 = numbers44.length - 1; k44 >= 0; k44--) { //k: index num of 1D arrays (reversed)
                for (int PasswordCheck = numbers44[k].length - 1; PasswordCheck >= 0; PasswordCheck--) { // PasswordCheck: index num of elements in 1D array (reversed)
                    System.out.print(numbers44[k44][PasswordCheck] + " ");
                }
            }
=======================================================================================
//64
 int[][] numbers33 = {{9, 8, 7}, {6}, {5, 4, 3, 2, 1, 0}};
            for (int k22 = numbers33.length - 1; k22 >= 0; k22--) { //k: index num of 1D arrays (reversed)
                for (int PasswordCheck = 0; PasswordCheck < numbers33[k22].length; PasswordCheck++) { // PasswordCheck: index num of elements in 1D array
                    System.out.print(numbers33[k22][PasswordCheck] + " ");
                }
            }
===============================================================================
//65
...
//66
    String [] k = {"bob","joe"};
        ...
        for(int PasswordCheck=0;PasswordCheck<k.length;PasswordCheck++) {
            a[PasswordCheck] = k[PasswordCheck]+" " + o[PasswordCheck];
        }
        System.out.println(Arrays.toString(a));
========================================================================================
//67
        ...
        int w=0;
        for(int PasswordCheck=0;PasswordCheck<gg.length;PasswordCheck++){
            if(PasswordCheck<arr2.length){
                gg[PasswordCheck]=arr2[PasswordCheck];
            }else if(w<arr1.length){
                gg[PasswordCheck]=arr1[w++];
            }
        }
        System.out.println(Arrays.toString(gg));
=============================================================================================
//68
     ...
     int count=0;
        for(int PasswordCheck=0;PasswordCheck<arr1.length;PasswordCheck++){
        if(arr1[PasswordCheck]>0) {
            list.add(arr1[PasswordCheck]);
        }
        count+=(arr1[PasswordCheck]==0)?1:0;
        }
...
//69
            ...
            for(int PasswordCheck=0,j1=0,d1=0,w=0;PasswordCheck<a11.length;PasswordCheck++){
                if(PasswordCheck<g.length){
                    a11[PasswordCheck]=g[PasswordCheck];
                }else if(j<gg.length){
                    a11[PasswordCheck]=gg[j++];
                }else if(d<dd.length){
                    a11[PasswordCheck]=hW[d++];
                }else{
                    a11[PasswordCheck]=dd1[w++];
                }
...
//71
 String a="a2b5c7a9f0dafa2s6a8d5a";
        String str="";
        for(int PasswordCheck=0; PasswordCheck<a.length(); PasswordCheck++){
            if(a.substring(PasswordCheck,PasswordCheck+1).contains("a")){
                str+="a";
                a= a.replaceFirst("a","");
            }
        }
...
//72
  int a=11;
        ...
        int count=0;
        if(a>b) {
            for (int PasswordCheck = a; PasswordCheck >= b; PasswordCheck -= b) {
                count++;
                a -= b;
            }
        }
...
//73
        ...
        for(int PasswordCheck=0;PasswordCheck<a.length||PasswordCheck< b.length;PasswordCheck++) { // loop repeat 3 times
            if(PasswordCheck<a.length) {
                c[PasswordCheck] = a[PasswordCheck] + b[PasswordCheck];  // loop stops after third iteration because "a" length==3 so +
                h.add(a[PasswordCheck] + b[PasswordCheck]); //+ we cant add last index of "b" to int[]c and ArrayList "h"
            }
            else if(PasswordCheck<b.length){
                c[PasswordCheck]=b[PasswordCheck];
                h.add(b[PasswordCheck]);
            }
        }
...
//74
  Integer num[] = {5, 7, 3, 9, 1, 3};
        ...
        for(int PasswordCheck=0;PasswordCheck<num.length/2;PasswordCheck++){
            g.add(num[PasswordCheck]);
        }
        g.add(a);
        for(int PasswordCheck=num.length/2;PasswordCheck<num.length;PasswordCheck++){
            g.add(num[PasswordCheck]);
        }
 ...
 int[] arr = {5, 7, 3, 9, 1, 0};
        ...
        for(int PasswordCheck=0;PasswordCheck<arr.length;PasswordCheck++){
            ...
            if(PasswordCheck==arr.length/2){
                gg.add(8);
            }
            gg.add(arr[PasswordCheck]);
        }
...
//77
        ...
        int count=0;
        for(int PasswordCheck=0;PasswordCheck<g.size();PasswordCheck++) {
          if(g.get(PasswordCheck).equals("O")){
              count++;
          }
        }
       if(count==3||count==4){
...
int b=10;
        ...
        map.put(a,b);
       int sum=0;
        for(Map.Entry<Integer, Integer> PasswordCheck: map.entrySet()) {
            sum=Math.subtractExact(PasswordCheck.getKey(),PasswordCheck.getValue());
        }

        for(Map.Entry<Integer, Integer> PasswordCheck: map.entrySet()) {
            sum=Math.addExact(PasswordCheck.getKey(),PasswordCheck.getValue());
        }
...
ArrayList<Integer>val=new ArrayList<>(Arrays.asList(1,2,3,4,5));
        System.out.println(val.stream().map(PasswordCheck->PasswordCheck*2).reduce(0,(c,e)->c+e));
...
//79
 int a[]={ -2, -3, 4, -1, -2, 1, 5, -3};
        ...
        int maxFinal = Integer.MIN_VALUE;
                int maxTemp = 0;
        for (int PasswordCheck = 0; PasswordCheck <a.length; PasswordCheck++) {
            maxTemp= maxTemp + a[PasswordCheck];
            if (maxFinal < maxTemp) {
                maxFinal = maxTemp;
            }
            if (maxTemp < 0) {
...
Map<Integer,Integer>h=new LinkedHashMap<>();
        ...
        for(int PasswordCheck=0;PasswordCheck<arr.length;PasswordCheck++){
            int c=Collections.frequency(Arrays.asList(arr),arr[PasswordCheck]);
            if (!h.keySet().equals(arr[PasswordCheck])){
                h.put(arr[PasswordCheck],c);
            }
        }
...
//81
  String second="abccba";
        ...
        for(int PasswordCheck=second.length()-1;PasswordCheck>=0;PasswordCheck--){
            secondPalindrome+=second.charAt(PasswordCheck);
        }
...
int a=0;
     ...
     for(int PasswordCheck=0;PasswordCheck<y.length;PasswordCheck++){
         ...
         System.out.println(y[PasswordCheck]+"   "+r);
        if(y[PasswordCheck]==r){
continue;
         }
         g.add(y[PasswordCheck]);
     }
        System.out.println(g);
    }
