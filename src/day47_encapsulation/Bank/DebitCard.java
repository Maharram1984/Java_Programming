...
/*  Debit card - custom classes + static
 ...
 /*  Create a constructor that will create a debit card object and initialize the card number,
     the card holder name, and the balance. The card number has to be 16 characters long
     to be a valid card number. If the length is too short or too big do not store
   ...
   public DebitCard(long cardNumber,String holderName,double balance){
       ...
       if(String.valueOf(cardNumber).length() == 12){  // ("" + cardNumber).length()
           this.cardNumber = cardNumber;
       } else{
           System.out.println("Invalid card number,the length must be 12");
       }
   }

  /* Overload the constructor to take all instance variables.
     ...
     The pin must be only 4 characters long. If the pin is more than or
    ...
    /* Add the toString method that will print all of cards information
